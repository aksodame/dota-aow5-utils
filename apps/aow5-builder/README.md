...
Static output — `pnpm build` writes everything to `apps/aow5-builder/dist/` (~25 MB, mostly the 1,045 item
icons, which Vite copies out of the shared package). Upload that directory and serve it; `pnpm preview`
serves it locally first.

**No host is configured yet** — a VPS is the plan. Nothing in the repo is tied to one: there is no deploy
script, no wrangler config and no provider dependency to remove later.

Two things any host needs to get right:

- Serve from the **domain root**, or the default `base` of `/` is wrong. For a subpath, build with
  `VITE_BASE=/sub/ pnpm build`.
...
apps/aow5-builder/        the planner UI
...
The extracted data and icons are the package's too, so `apps/aow5-builder/vite.config.ts` points `publicDir`
...
`src/components/fx/` adds a motion layer in the spirit of [React Bits](https://reactbits.dev) — an aurora
backdrop, count-ups and staggered reveals. These are written from scratch
rather than vendored: React Bits ships under MIT **+ Commons Clause**, and keeping that out of the tree keeps
this project's licensing simple. They are plain CSS animations with no new dependencies, and every one is
paired with `motion-reduce:animate-none` so `prefers-reduced-motion` is respected.
