...
`apps/aow5-tracker/capture/` is gitignored. If a PR needs evidence from a real session, paste the handful
of `[AOW5TRK]` lines that matter, with the slot ids scrubbed — never attach the whole file.

`apps/aow5-tracker/private/` is correspondence with the addon's developer and is not public. Anything
