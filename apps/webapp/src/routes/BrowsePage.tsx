...
import { OLD_SITE_BUILDS, oldBuildsUrl } from '@/lib/links';
...
export function BrowsePage({
  ...
  const { strings, core, lang } = useApp();
  ...
  return (
    <div className={styles.page}>
      {/*
        Above both columns, because it is addressed to somebody who has not
        come here to search: this list starts empty of everything the old site
        holds, and only an author can move their own builds across. It spans
        the grid rather than sitting in either column — inside the list it
        would read as something about the results, and inside the sidebar as
        another filter.

        The link carries `?lang=` for the same reason the tracker's does: the
        old site is the previous version of this one and reads it the same way.
      */}
      <aside className={styles.notice}>
        {strings.browse.notice}{' '}
        <a className={styles.noticeLink} href={oldBuildsUrl(lang)} target="_blank" rel="noreferrer noopener">
          {/* The address itself, without the scheme — it is the thing being
              pointed at, and a label like "the old site" would hide which one. */}
          {OLD_SITE_BUILDS.replace(/^https?:\/\//, '')}
