...
export const ru: Messages = {
  ...
  settings: {
    ...
    sounds: {
      ...
      enabledHint:
        'Звучит один раз на каждый подобранный предмет с привязкой. «Алое сердце» привязано к джекпоту из коробки; отвяжете — так и останется.',
      volume: 'Громкость',
    ...
    log: {
      ...
      cache: 'Очистить кэш',
      cacheHint:
        'Выбрасывает загруженные иконки предметов и заново открывает панели. Обновление делает это само; нажмите, если в промежутке иконки пустые или битые: неверный ответ сервера может пролежать в кэше неделями.',
    },
    rooms: {
      title: 'По комнатам',
      room: 'комната',
